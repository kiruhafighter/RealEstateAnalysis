...
namespace RealEstateAnalysis.Client.Pages
{
    public class ChatModel : PageModel
    {
        ...
        [BindProperty] public List<ChatMessage> ChatHistory { get; set; } = new();
        ...
        public async Task<IActionResult> OnPostAsync(string clear)
        {
            ...
            {
                ChatHistory = System.Text.Json.JsonSerializer.Deserialize<List<ChatMessage>>(chatHistoryJson) ??
            ...
            if (!string.IsNullOrWhiteSpace(UserMessage))
            {
                ...
                {
                    ChatHistory.Add(new ChatMessage { role = "assistant", content = response.response });
                }

                if (response?.suggested_property_ids?.Count > 0)
                {
